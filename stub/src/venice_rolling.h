 ...
 *
 * This header + venice_rolling.c are compiled into the stub ONLY when
 * VVM_ROLLING is defined. With it undefined the file is inert and the default
 * build is byte-identical to today.
 */
#ifndef ORION_VENICE_ROLLING_H
#define ORION_VENICE_ROLLING_H
...
#define VVM_ROLL_MAGIC0 'V'
#define VVM_ROLL_MAGIC1 'R'
#define VVM_ROLL_SEED_LEN 16
#define VVM_ROLL_MAX_INSTR 9   /* push_imm64 = opcode + 8 operand bytes */
...
typedef struct VvmRolling {
    const uint8_t *ct;
    uint32_t       ct_len;
    uint8_t        seed[VVM_ROLL_SEED_LEN];
 ...
 * (out_plain must be >= VVM_ROLL_MAX_INSTR bytes). Reseeds the accumulator when
 ...
 * matching venice_vm_exec's -1 discipline.
 *
 ...
 * stub init under VVM_ROLLING to fail fast on a port regression.
 */
