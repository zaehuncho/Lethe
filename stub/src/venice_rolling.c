 ...
 *
 * Compiled only when VVM_ROLLING is defined (see CMakeLists.txt gate).
 */
#ifdef VVM_ROLLING

#include "venice_rolling.h"
#include "crypto.h"   /* crypto_sha256 */
#ifdef VVM_ROLL_POISON
#include <intrin.h>   /* __readgsqword for the PEB read */
#endif

#define VVM_ROLL_GOLDEN 0x9E3779B97F4A7C15ULL

/* Operand width by CANONICAL opcode byte (post-unmap). Everything not listed
 * is 0. Must match venice_disasm.OPCODE_TABLE widths. A switch (not a
 * designated-initializer table) keeps this portable across every MSVC C mode. */
static uint32_t vvm_roll_width(uint8_t canon_op)
{
    switch (canon_op) {
...
static int vvm_roll_opcode_valid(uint8_t op)
{
    return op <= 0x37;
}

#ifdef VVM_SHUFFLED
#include "venice_opcodes_shuffled.h"   /* VVM_OPCODE_UNMAP[256] */
#endif
...
uint64_t vvm_roll_resync(const uint8_t seed[VVM_ROLL_SEED_LEN], uint32_t leader)
{
    uint8_t buf[VVM_ROLL_SEED_LEN + 1 + 4];
    uint8_t dig[32];
    int i;
    for (i = 0; i < VVM_ROLL_SEED_LEN; i++) buf[i] = seed[i];
    buf[VVM_ROLL_SEED_LEN] = 'R';
    put_u32le(buf + VVM_ROLL_SEED_LEN + 1, leader);
    if (crypto_sha256(buf, sizeof(buf), dig) != 0) return 0;
    return load_u64le(dig);
}

/* keystream(seed, pc, acc) -> first VVM_ROLL_MAX_INSTR bytes of the SHA256. */
static int vvm_roll_keystream(const uint8_t seed[VVM_ROLL_SEED_LEN],
                              uint32_t pc, uint64_t acc,
                              uint8_t out[VVM_ROLL_MAX_INSTR])
{
    uint8_t buf[VVM_ROLL_SEED_LEN + 1 + 4 + 8];
    uint8_t dig[32];
    int i;
    for (i = 0; i < VVM_ROLL_SEED_LEN; i++) buf[i] = seed[i];
    buf[VVM_ROLL_SEED_LEN] = 'K';
    put_u32le(buf + VVM_ROLL_SEED_LEN + 1, pc);
    put_u64le(buf + VVM_ROLL_SEED_LEN + 1 + 4, acc);
    if (crypto_sha256(buf, sizeof(buf), dig) != 0) return -1;
    for (i = 0; i < VVM_ROLL_MAX_INSTR; i++) out[i] = dig[i];
    return 0;
}

uint64_t vvm_roll_fold(uint64_t acc, const uint8_t *plain, uint32_t len,
                       uint32_t pc)
{
    uint32_t i;
    for (i = 0; i < len; i++) {
        acc = rotl64(acc ^ (uint64_t)plain[i], 7);
        acc = acc + VVM_ROLL_GOLDEN;
    }
    acc ^= (uint64_t)pc;
    return acc;
}

void vvm_rolling_init(VvmRolling *r, const uint8_t *code_ct, uint32_t ct_len,
                      const uint8_t seed[VVM_ROLL_SEED_LEN],
                      const uint8_t *leaders, uint16_t n_leaders)
{
    int i;
    r->ct = code_ct;
    r->ct_len = ct_len;
    for (i = 0; i < VVM_ROLL_SEED_LEN; i++) r->seed[i] = seed[i];
    r->leaders = leaders;
...
static uint32_t vvm_roll_leader_at(const VvmRolling *r, uint16_t i)
{
...
static int vvm_roll_is_leader(const VvmRolling *r, uint32_t pc)
{
    uint16_t i;
    for (i = 0; i < r->n_leaders; i++) {
        if (vvm_roll_leader_at(r, i) == pc) return 1;
    }
    return 0;
}

/*
 * OBSERVATION-POISON (VVM_ROLL_POISON): fold an anti-instrumentation signal into
 ...
 * Kill-switch: the whole thing compiles out when VVM_ROLL_POISON is undefined.
 */
static uint64_t vvm_roll_env_poison(void)
{
#ifdef VVM_ROLL_POISON
...
int vvm_rolling_fetch(VvmRolling *r, uint32_t pc,
                      uint8_t out_plain[VVM_ROLL_MAX_INSTR], uint32_t *out_len)
{
    uint8_t ks[VVM_ROLL_MAX_INSTR];
    uint8_t wire_op, canon_op;
    uint32_t width, ilen, i;

    if (pc >= r->ct_len) return -1;
    if (vvm_roll_is_leader(r, pc))
        r->acc = vvm_roll_resync(r->seed, pc) ^ vvm_roll_env_poison();
    if (vvm_roll_keystream(r->seed, pc, r->acc, ks) != 0) return -1;

    /* decrypt the opcode first, determine width, then the operands */
    wire_op = (uint8_t)(r->ct[pc] ^ ks[0]);
#ifdef VVM_SHUFFLED
    canon_op = VVM_OPCODE_UNMAP[wire_op];
#else
    canon_op = wire_op;
#endif
    if (!vvm_roll_opcode_valid(canon_op)) return -1;
    width = vvm_roll_width(canon_op);
    ilen = 1u + width;
    ...
    r->acc = vvm_roll_fold(r->acc, out_plain, ilen, pc);
    *out_len = ilen;
    /* wipe the local keystream (derived material) -- not elided (volatile). */
    {
        volatile uint8_t *z = (volatile uint8_t *)ks;
        uint32_t i2;
        for (i2 = 0; i2 < VVM_ROLL_MAX_INSTR; i2++) z[i2] = 0;
    }
    return 0;
}

/* ---- self-test: bit-exact agreement with the Python oracle -------------- */
int vvm_rolling_selftest(void)
{
    uint8_t seed[16];
    int i;
    for (i = 0; i < 16; i++) seed[i] = (uint8_t)i;

    /* pinned vectors (see venice_rolling.py output baked into the harness) */
    if (vvm_roll_resync(seed, 0) != 0x0E17E9881DD39855ULL) return -1;
    if (vvm_roll_resync(seed, 7) != 0xA30D3A759BCBB752ULL) return -1;
    {
        uint8_t two[2]; two[0] = 1; two[1] = 2;
        if (vvm_roll_fold(0, two, 2, 0) != 0xB9F456792488C7E4ULL) return -1;
    }
    {
        uint8_t thr[3]; thr[0] = 2; thr[1] = 9; thr[2] = 9;
        if (vvm_roll_fold(vvm_roll_resync(seed, 0), thr, 3, 0)
            != 0xC96670BECE6DEDEFULL) return -1;
    }
    {
        uint8_t ks[VVM_ROLL_MAX_INSTR];
        if (vvm_roll_keystream(seed, 0, vvm_roll_resync(seed, 0), ks) != 0)
            return -1;
