...
|---|-----------|-----------|------------------------------|
| **SP1** | **Rolling state accumulator (P1)** | A 64-bit value + a SHA/ARX midstate, folded at the `dvm_push`/`dvm_pop` choke points and at each dispatched `opcode‖operand‖sp‖taken-edge`. The single keystream/decode seed. | The one thing every "value-keyed" and "history-keyed" scheme reads from. |
| **SP2** | **Memguard v3 + VM-internal key schedule (P2)** | Per-page (and, on crown-jewel routines, per-basic-block) AEAD; **AES key expansion moved *inside* the Daedalus VM** so round keys are consumed fragment-wise and **no contiguous 32-byte key ever exists in a register or stack cell**; short randomized re-encrypt jitter; decoy resident pages. | **The single most-repeated red-team fix.** Kills `key_scatter_get()`'s contiguous reassembly (`key_scatter.c:175`) and the whole-section plaintext flash (`memguard.c:463`) — the two facts every "just snapshot the key/section" attack depends on. |
...
| **SP5** | **Per-build seed + differential pack-time harness + kill-switches** | One seed threaded through `daedalus_asm.py` / `daedalus_vm.c` / `daedalus_disasm.py` / `shuffle_opcodes.py`. Assembler-simulated result must equal interpreter result **bit-exactly on a clean build or the *pack* fails — never the customer**. Universal flags: `--tamper-fold={off,env,timing}`, `--deception={off,on}`, per-feature bits in `PackInfo.flags`. | The safety net for every rolling/entangled/keyed scheme. No hardened build ships without AV-smoke + clean-VM A/B. |
...
- **VM-internal AES-256 key schedule.** Port `aes256_key_expand` / `aes256_encrypt_block` (`crypto.c:263,320`) into Daedalus native ops that read fragments directly from the scatter pages and expand round keys **without** ever assembling `out_key[32]`. `key_scatter_get` as a public "give me the whole key" primitive is deleted; only `key_scatter_feed_schedule()` (fragment → round-key word) survives.
...
## 3. Track 1 — Daedalus VM overhaul
...
**Touch.** `daedalus_vm.c` (fetch/reseal, multi-site dispatch), `daedalus_asm.py` (forward-simulating encoder + triangular solver), `daedalus_disasm.py` (matching emulator for the differential harness). `code[]` lives in a **private RW `VirtualAlloc` page** (data SMC — *not* `.text`, stays AV-clean).

**Container/ABI.** None (bytecode ships inside the stub as `daedalus_programs.h`, not in the container).
...
**Touch.** `daedalus_vm.c` (pushdown+mode decode, closed-form successor), `daedalus_asm.py`/`daedalus_disasm.py` (must simulate pushdown+mode — a balance bug bricks the crypto path, so the differential harness is mandatory and load-bearing). `crypto_derive_key` path (`crypto.c:474`) gains the path-tag HKDF term.
...
**Touch.** `daedalus_vm.c`, `daedalus_trampoline.asm` (must preserve the x64 nonvolatile/shadow-space contract), `crypto.c` (native crypto byte-range hashing), `daedalus_asm.py` (relocation-stable folded ranges, rebaked per build).
...
*"Denuvo's actual thesis minus the fragile binary lifter: the owner MARKS a load-bearing first-party function and it ships only as Daedalus bytecode — devirtualize it wrong and the app computes wrong answers, not a dead key."*

**Mechanism (hardened).** Do **not** lift a binary (brick-prone). Compile from **source**: an `ORNVM_VIRT` macro extracts an audited pure-integer/POD kernel into `daedalus_asm`, and payload call sites re-enter `daedalus_vm_exec` through a native→VM thunk, so genuine app output flows through the interpreter. **Hardened against both fatal bypasses the red-team found:** (1) **secret-share, don't N-version-vote** — the original "K implementations + a native path, XOR-combined" *requires a plaintext native participant that computes the answer*; instead each of K paths computes a random additive/XOR **share** `sᵢ`, with `V = ⊕sᵢ` only at the fold, so reading any single path (native included) yields noise while a wrong share still corrupts `V`; (2) **destroy purity / inflate the domain** — fold the code-hash-bound key, a wide slab of decrypted `.rdata`, and the live epoch/PRP state into the kernel, and have the VM read app state directly by pointer (`DVM_LOAD64` over live structs) rather than through one clean typed thunk, so `(visible-args → result)` is *not a learnable function* and I/O-sampling/synthesis cannot tabulate it. Rotate the opcode PRP on a coarse epoch cadence (the memguard sweeper) so a live interpreter snapshot goes stale; wrap the epoch swap in a seqlock.
...
**Touch.** `daedalus_vm.c` (two opcodes), `daedalus_asm.py`, `container.py` (baked canonical band boundaries in the expectations blob).
...
**Touch.** New `stub/src/dvm_blocks.c` (block decrypt/execute/reseal), `daedalus_vm.c` (block-threaded fetch), `daedalus_asm.py` (block sealing + hash-chain, must match the runtime path exactly), `crypto.c` (reuse SHA256/HKDF).
...
**Touch.** `memguard.c` (`mg_reencrypt_page`, `mg_activate_page`, the `keys[][32]` table), `crypto.c`/`lethe_derive_section_key` (`crypto.c:494` → ratchet), `pe_loader.c` (`decrypt_section` eager-section wipe), `daedalus_vm.c` (VM-hosted pad derivation).
...
**Touch.** New `stub/src/at_rest.c` (encrypted-field accessor), `key_scatter.c` (migrate/relocate), `antidebug.c` (heartbeat/suspend detection), `daedalus_vm.c` (VM-hosted schedule).
...
**Touch.** New `packer/gather.py` (deterministic last-step gather-list builder + verify gate), `daedalus_vm.c` (on-demand gather fetch), `container.py`/`pack_info.h` (gather-list region).
...
**Touch.** `pe_loader.c` (`resolve_imports`, IAT masking, single dispatcher), new `stub/src/iat_gate.c` (dispatcher + per-call token), `antidebug.c` (ntdll stub tomography → SP4), `daedalus_vm.c` (import-as-native-op).
...
**Touch.** `key_scatter.c` (honeypot blob), `daedalus_strenc.py`/`daedalus_str_data.h` (perjured strings), `memguard.c` (resident decoy pages), new `packer/decoys.py` (build-time verifier that no edge/reloc targets a real path *and* that decoy coverage/taint braids correctly), `daedalus_asm.py` (MBA cancel-identity).
...
**Touch.** `daedalus_vm.c` (lane-wise arithmetic, fused Garner-to-consumer), `daedalus_asm.py`/`daedalus_disasm.py` (encode/decode residues, rolling moduli), `crypto.c` (moduli from code-hash).
...
**Touch.** `daedalus_asm.py` (laundering codegen + MBA predicates), `daedalus_vm.c` (temporal store + VM AES schedule), `key_scatter.c` (presence-pattern reassembly).
...
**Touch.** `daedalus_asm.py` (predicate library — each numerically unit-tested at pack across environments or it universally bricks), `daedalus_vm.c` (lazy per-page fold), `key_scatter.c`, `crypto.c` (code-hash+section measurement seed).
...
**Touch.** `memguard.c` (VEH observation of RDPMC #GP, per-page oracle binding), new `stub/src/emu_oracles.c`, `daedalus_vm.c` (AESENC fold, provenance fold), `crypto.c`.
...
**Touch.** `antidebug.c` (check subset → fold, ring wiring), `crypto.c`/`crypto_derive_key` (`crypto.c:474`, ring digest as HKDF info), `daedalus_vm.c` (VM-hosted key), `container.py` (baked baselines).
...
**Touch.** New `packer/metamorph.py` (source pass) + `packer/watermark.py` (id ledger), all three `daedalus_*` tools, `pe_loader.c`/`antidebug.c` (variant bodies), payload-side watermark hook.
...
**Touch.** `daedalus_trampoline.asm` (positive-SP reconverge), new `packer/unwind_poison.py` (perjured `UNWIND_INFO` on decoy frames + post-build unwind simulator), `container.py` (decoy `.pdata`).
...
- **Deliverable:** SP1 rolling accumulator plumbed at `dvm_push`/`dvm_pop`; per-build seed threaded through all `daedalus_*` tools; SP5 differential pack-time harness (assembler vs interpreter, bit-exact or pack fails); kill-switch flags reserved in `PackInfo.flags`; SP4 deception-router skeleton (selects between two valid blobs).
- **Files:** `daedalus_vm.c`, `daedalus_asm.py`, `daedalus_disasm.py`, `shuffle_opcodes.py`, `container.py` (flag bits only), `pack_info.h` (flag bits, still 192 B).
...
- **Deliverable:** §3.1 history-keyed tape (RW page), §3.2 stateful/tableless dispatch (kills the readable `switch`), §3.3 cross-domain seal, §6.1 RNS stack, §6.2 control-to-data laundering. All runtime/bytecode-only (bytecode ships in `daedalus_programs.h`, not the container).
- **Files:** `daedalus_vm.c`, `daedalus_asm.py`, `daedalus_disasm.py`, `daedalus_trampoline.asm`, `crypto.c` (path-tag/seal HKDF terms).
...
- **Files:** `memguard.c`, `key_scatter.c`, `crypto.c`, `pe_loader.c`, `daedalus_vm.c`, **`container.py` + `pack_info.h` (v2, lockstep)**, new `stub/src/at_rest.c`.
...
- **Files:** `antidebug.c`, `crypto.c`/`crypto_derive_key`, `daedalus_vm.c`, new `stub/src/dvm_blocks.c` + `stub/src/cadence.c`, `container.py` (expectations blob populate — no layout change), `daedalus_asm.py`.
...
- **Files:** `stub_main.c`, `pe_loader.c`, `memguard.c`, `crypto.c`, `key_scatter.c`, `daedalus_strenc.py`, new `packer/decoys.py` + `packer/unwind_poison.py` (with the existential post-build unwind simulator), `container.py` (decoy region populate).
...
- **Files:** new `stub/src/uarch_fuse.c` + `emu_oracles.c` + `exc_weave.c` + `iat_gate.c`, `pe_loader.c`, `memguard.c`, `daedalus_vm.c`, new `packer/gather.py` + `packer/mortar.py`, `container.py` (gather/mortar regions populate).
...
- **Files:** new `packer/metamorph.py` + `packer/watermark.py`, all `daedalus_*` tools, `pe_loader.c`/`antidebug.c` (variant bodies), payload watermark hook, `reserved[]` build-id field.
