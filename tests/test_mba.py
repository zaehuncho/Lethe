#!/usr/bin/env python3
"""
Correctness proof for the MBA rewriter (venice/venice_mba.py).
...
import venice_mba            # noqa: E402
from venice_ref import run_plaintext  # noqa: E402
...
def _run(src, args):
    val, _trace = run_plaintext(venice_asm.assemble(src), args)
    return val
...
def test_xor_expansion_matches_random():
    rng = random.Random(1)
    src = _binop_prog("xor")
    rewritten = venice_mba.rewrite_source(src, ops=("xor",))
    assert "MBA[xor]" in rewritten and venice_mba.count_rewritable(src) == 1
    for _ in range(200):
...
def test_add_expansion_matches_random():
    rng = random.Random(2)
    src = _binop_prog("add")
    rewritten = venice_mba.rewrite_source(src, ops=("add",))
...
def test_composed_and_nested():
    ...
    zero:
      ...
      halt
    """
    rewritten = venice_mba.rewrite_source(src, ops=("xor", "add"))
    assert venice_mba.count_rewritable(src) == 3
    rng = random.Random(3)
    for _ in range(200):
        a = rng.getrandbits(64); b = rng.getrandbits(64)
...
def test_idempotent_structure_on_real_programs():
    ...
    import venice_disasm
    for vasm in sorted(PROGRAMS_DIR.glob("*.vasm")):
        text = vasm.read_text(encoding="utf-8")
        rewritten = venice_mba.rewrite_source(text)
        blob = venice_asm.assemble(rewritten)          # must assemble
        _txt = venice_disasm.disassemble(blob)          # must disassemble
        # data section is untouched
        orig_ds = struct.unpack_from("<H", venice_asm.assemble(text), 0)[0]
...
def test_mba_composes_with_rolling():
    ...
    src = _binop_prog("add")
    rewritten = venice_mba.rewrite_source(src)
    blob = venice_asm.assemble(rewritten)
